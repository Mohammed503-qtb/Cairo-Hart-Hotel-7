...
class HotelStore extends ChangeNotifier {
  ...
  void _seed() {
    ...
    // Staff access codes — generated by the admin from the admin control
    // panel (PLAN §4.1, §37). Seeded here so the unified login flow is
    // demonstrable. The admin code bootstraps the admin panel; the reception
    // code is one the admin "created" for the receptionist.
    staffAccesses.add(
      StaffAccess(
        id: 'sa-admin-1',
        code: 'ADM-100',
  ...
  /// Validate a staff access code. Returns the matching StaffAccess if the
  /// code is active and not expired, else null. Also updates lastUsedAt.
  StaffAccess? validateStaffCode(String code) {
    final sa = firstWhereOrNull(
      staffAccesses,
      (a) => a.code == code && a.active,
    );
  ...
  /// admin. Returns the generated StaffAccess.
  StaffAccess createStaffCode({
    required String staffName,
    required String role,
    int validityDays = 90,
  }) {
    final prefix = role == 'admin' ? 'ADM' : 'REC';
    final code = '$prefix-${++_staffCodeSeq}';
  ...
  void regenerateStaffCode(String id) {
    final sa = firstWhereOrNull(staffAccesses, (a) => a.id == id);
    if (sa == null) return;
    final prefix = sa.role == 'admin' ? 'ADM' : 'REC';
    // Replace the object since `code` is final — find index and swap.
    final newCode = '$prefix-${++_staffCodeSeq}';
