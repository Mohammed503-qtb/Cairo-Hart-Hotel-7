              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                          ...
                          children: [
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                StatusChip(label: '${l.roomLabel} ${room?.number}', color: theme.colorScheme.primary),
                                StatusChip(label: guest?.name ?? '', color: const Color(0xFF00838F)),
                            ...
                            Text('${l.isArabic ? "أُنشئ" : "Created"} ${Fmt.dateTime(r.createdAt)}',
                                style: theme.textTheme.bodySmall?.copyWith(color: theme.colorScheme.onSurface.withOpacity(0.6))),
                            if (r.assignedTo != null)
                              Text('${l.isArabic ? "مُسند إلى" : "Assigned"}: ${r.assignedTo}',
                                  style: theme.textTheme.bodySmall),
                            if (r.completedAt != null)
                              Text('${l.isArabic ? "أُنجز" : "Completed"} ${Fmt.dateTime(r.completedAt!)}',
                                  style: theme.textTheme.bodySmall),
                          ],
                        ),
                      ),
                    ...
                    Text(l.isArabic ? 'إجراءات' : 'Actions', style: theme.textTheme.titleMedium),
                    const SizedBox(height: 8),
                    Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: [
                        ActionChip(
                          ...
                          onPressed: r.status == RequestStatus.newRequest
                              ? () => store.updateRequestStatus(r.id, RequestStatus.acknowledged)
                              : null,
                        ),
                        ActionChip(
                          label: Text(l.assign),
                          avatar: const Icon(Icons.person_add_alt_1_outlined),
                          onPressed: r.status == RequestStatus.acknowledged || r.status == RequestStatus.newRequest
                              ? () => _assign(r)
                              : null,
                        ),
                        ActionChip(
                          ...
                          onPressed: r.status == RequestStatus.acknowledged || r.status == RequestStatus.assigned
                              ? () => store.updateRequestStatus(r.id, RequestStatus.inProgress)
                              : null,
                        ),
                        ActionChip(
                          label: Text(l.markComplete),
                          avatar: const Icon(Icons.check),
                          onPressed: r.status != RequestStatus.completed && r.status != RequestStatus.cancelled
                              ? () => store.updateRequestStatus(r.id, RequestStatus.completed)
                              : null,
                        ),
                        ActionChip(
                          label: Text(l.cancel),
                          ...
                          onPressed: r.status != RequestStatus.completed && r.status != RequestStatus.cancelled
                              ? () => store.updateRequestStatus(r.id, RequestStatus.cancelled)
                              : null,
                        ),
                      ],
                    ),
                    const SizedBox(height: 16),
                    Text(l.isArabic ? 'المحادثة' : 'Conversation', style: theme.textTheme.titleMedium),
                    const SizedBox(height: 8),
                        ...
                        return Align(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: mine ? theme.colorScheme.primary : theme.colorScheme.onSurface.withOpacity(0.08),
                            ...
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(m.text, style: TextStyle(color: mine ? Colors.white : theme.colorScheme.onSurface)),
                                Text(Fmt.time(m.at), style: TextStyle(fontSize: 10, color: mine ? Colors.white70 : theme.colorScheme.onSurface.withOpacity(0.5))),
                              ],
                            ),
                          ),
                        );
              ...
              child: Row(
                children: [
                  Expanded(
                    child: TextField(
                      controller: _msgCtrl,
                      decoration: InputDecoration(hintText: l.isArabic ? 'رد على النزيل...' : 'Reply to guest...'),
                    ),
                  ),
                  const SizedBox(width: 8),
                  IconButton.filled(
                    onPressed: () {
                      ...
                      store.addRequestMessage(r.id, RequestMessage(
    ...
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: InputDecoration(hintText: l.isArabic ? 'مثال: التنظيف - ليلى' : 'e.g. Housekeeping — Layla'),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.of(c).pop(), child: Text(l.cancel)),
          FilledButton(
            onPressed: () {
              final who = _assignCtrl.text.trim().isEmpty ? 'Unassigned' : _assignCtrl.text.trim();
              store.updateRequestStatus(r.id, RequestStatus.assigned, assignedTo: who);
              _assignCtrl.clear();
              Navigator.of(c).pop();
            },
            child: Text(l.assign),
