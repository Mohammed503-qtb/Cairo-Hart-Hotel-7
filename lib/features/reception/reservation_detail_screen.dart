...
class _ReservationDetailScreenState extends State<ReservationDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final assignedRoom = res.assignedRoomId == null ? null : store.roomById(res.assignedRoomId!);
    final alreadyCheckedIn = res.status == ReservationStatus.checkedIn;
    final stay = res.stayId == null ? null : store.stayById(res.stayId!);
    final access = stay?.guestAccessId == null ? null : store.firstWhereOrNull(store.accesses, (a) => a.id == stay!.guestAccessId);

    // candidate rooms for assignment
    final candidates = store.rooms.where((r) {
      if (r.roomTypeId != type.id) return false;
      if (!r.active) return false;
      if (r.status == RoomStatus.outOfOrder || r.status == RoomStatus.outOfService) return false;
      if (r.status == RoomStatus.occupied) {
        return r.currentStayId != null; // allow to show but not selectable
      }
      return true;
    }).toList();
    _selectedRoomId = _selectedRoomId ?? assignedRoom?.id ?? (candidates.isEmpty ? null : candidates.first.id);
            ...
            children: [
              ...
              Card(
                color: res.status == ReservationStatus.cancelled
                    ...
                    : (alreadyCheckedIn ? const Color(0xFF2E7D32).withOpacity(0.08) : theme.colorScheme.primary.withOpacity(0.05)),
                child: Padding(
                  padding: const EdgeInsets.all(16),
                  child: Row(
                    children: [
                      Icon(alreadyCheckedIn ? Icons.check_circle : Icons.event, color: alreadyCheckedIn ? const Color(0xFF2E7D32) : theme.colorScheme.primary),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('${res.id} • ${Fmt.dateShort(res.checkIn)} → ${Fmt.dateShort(res.checkOut)}',
                                style: theme.textTheme.bodyMedium),
                          ],
                        ),
                      ),
                      StatusChip(label: res.status.label, color: alreadyCheckedIn ? const Color(0xFF2E7D32) : const Color(0xFF1565C0)),
                    ],
                  ),
                ),
              ),
              ...
              SectionCard(
                ...
                child: Column(
                  children: [
                    _row(l.isArabic ? 'الغرفة' : 'Room type', l.isArabic ? type.nameAr : type.name),
                    _row(l.checkInDate, Fmt.date(res.checkIn)),
                    _row(l.checkOutDate, Fmt.date(res.checkOut)),
                    _row(l.isArabic ? 'المدة' : 'Duration', '${res.price.nights} ${res.price.nights == 1 ? l.night : l.nights}'),
                    _row(l.isArabic ? 'النزلاء' : 'Guests', '${res.adults} ${l.adults.toLowerCase()}' + (res.children > 0 ? ' + ${res.children} ${l.children.toLowerCase()}' : '')),
                    const Divider(),
              ...
              if (alreadyCheckedIn && stay != null) ...[
                SectionCard(
                  ...
                  child: Column(
                    children: [
                      ...
                      _row(l.roomLabel, store.roomById(stay.roomId)?.number ?? ''),
                      _row(l.isArabic ? 'الحالة' : 'Status', stay.status.label),
                      if (access != null)
                        _row(l.isArabic ? 'رمز الدخول' : 'Access code', access.code, bold: true),
                      _row(l.outstandingBalance, Fmt.money(store.outstandingBalance(stay.id)), color: store.outstandingBalance(stay.id) > 0 ? const Color(0xFFEF6C00) : const Color(0xFF2E7D32)),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => context.push('/reception/checkout/${stay.id}'),
              ...
              ] else if (res.status == ReservationStatus.confirmed || res.status == ReservationStatus.pending) ...[
                // CHECK-IN WORKFLOW
                SectionCard(
                  title: l.verifyGuest,
                  child: Column(
                    ...
                    children: [
                      ...
                      if (guest?.nationality != null) _row(l.isArabic ? 'الجنسية' : 'Nationality', guest!.nationality!),
                      const SizedBox(height: 10),
                      TextField(
                        controller: _idCtrl,
                        decoration: InputDecoration(labelText: l.isArabic ? 'رقم الهوية / الجواز' : 'ID / Passport number'),
                      ),
                    ],
                  ),
                ),
                    ...
                    children: [
                      GridView.count(
                        ...
                        children: candidates.map((r) {
                          ...
                          return Card(
                            color: sel ? theme.colorScheme.primary.withOpacity(0.12) : null,
                            child: InkWell(
                              onTap: blocked ? null : () => setState(() => _selectedRoomId = r.id),
                ...
                const SizedBox(height: 16),
                SectionCard(
                  title: l.isArabic ? 'دفعة مقدمة (اختياري)' : 'Deposit (optional)',
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(
                        children: [
                          Expanded(
                            child: TextField(
                              ...
                              onChanged: (v) => setState(() => _deposit = double.tryParse(v) ?? 0),
                            ),
                          ),
                          const SizedBox(width: 12),
                          DropdownButton<PaymentMethod>(
                            value: _depositMethod,
                            onChanged: (m) => setState(() => _depositMethod = m!),
                            items: [
                              ...
                              DropdownMenuItem(value: PaymentMethod.payAtHotel, child: Text(l.payAtHotel)),
                            ],
                          ),
                        ],
                      ),
              ...
              ] else if (res.status == ReservationStatus.cancelled || res.status == ReservationStatus.noShow) ...[
                SectionCard(
                  title: l.isArabic ? 'الحالة' : 'Status',
                  child: Text(res.status == ReservationStatus.cancelled
                      ? (l.isArabic ? 'ملغى' : 'Cancelled') + (res.cancellationReason != null ? ' • ${res.cancellationReason}' : '')
  ...
  void _doCheckIn(Reservation res, BuildContext context, L10n l) async {
    ...
    await showDialog(
      ...
      builder: (c) => AlertDialog(
        title: Row(
          children: [const Icon(Icons.check_circle, color: Color(0xFF2E7D32)), const SizedBox(width: 8), Text(l.isArabic ? 'تم تسجيل الدخول' : 'Checked-in')],
        ),
        content: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Text(access?.code ?? '', style: Theme.of(context).textTheme.headlineMedium?.copyWith(letterSpacing: 6, fontWeight: FontWeight.w800)),
            ),
            const SizedBox(height: 8),
            Text(l.isArabic ? 'شارك هذا الرمز مع النزيل عبر WhatsApp.' : 'Share this code with the guest via WhatsApp.',
                style: Theme.of(context).textTheme.bodySmall),
          ],
        ),
        actions: [
          FilledButton(onPressed: () => Navigator.of(c).pop(), child: Text(l.done)),
        ],
      ),
    );
    if (!mounted) return;
