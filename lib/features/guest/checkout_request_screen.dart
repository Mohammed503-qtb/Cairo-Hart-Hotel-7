...
class CheckoutRequestScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingReqs = store.requestsForStay(stay.id).where((r) =>
        r.status != RequestStatus.completed &&
        r.status != RequestStatus.cancelled &&
        r.status != RequestStatus.rejected).length;
            ...
            children: [
              SectionCard(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _row(context, l.isArabic ? 'المغادرة' : 'Checkout', Fmt.date(stay.checkOut)),
                    _row(context, l.outstandingBalance, Fmt.money(balance),
                        color: balance > 0 ? const Color(0xFFEF6C00) : const Color(0xFF2E7D32)),
                    _row(context, l.requests, '$pendingReqs ${l.isArabic ? "معلّق" : "open"}'),
                    _row(context, l.isArabic ? 'الحالة' : 'Status',
                        stay.status == StayStatus.checkoutPending ? l.pendingReview : stay.status.label),
                  ],
                ),
              ),
                  ...
                  onPressed: stay.status == StayStatus.checkoutPending
                      ...
                      : () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(l.isArabic ? 'تم إرسال طلب المغادرة ✓' : 'Checkout request sent ✓')),
                          );
                          context.pop();
                        },
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text(v, style: theme.textTheme.titleSmall?.copyWith(color: color, fontWeight: FontWeight.w700)),
        ],
      ),
    );
  }
